...
CREATE TABLE car_lot (
    brand VARCHAR2(100),
    c_name VARCHAR2(100),
    c_owner VARCHAR2(100) CONSTRAINT fk_owner REFERENCES user_acc(username),
    c_year NUMBER(4),
    price NUMBER(6),
    license VARCHAR2(100) PRIMARY KEY
);

CREATE TABLE offer_tab(
    customer VARCHAR2(100) CONSTRAINT fk_offer_owner REFERENCES user_acc(username),
    license VARCHAR2(100) CONSTRAINT fk_license REFERENCES car_lot(license),
    amount NUMBER(6),
    PRIMARY KEY (customer,license)
);
...
SELECT brand, c_name, c_owner, c_year, price, license FROM car_lot WHERE c_owner != 'DLR';

/
create or replace TRIGGER on_car_delete
AFTER DELETE ON car_lot
FOR EACH ROW
BEGIN
    DELETE FROM offer_tab WHERE license = :OLD.license;
END;
